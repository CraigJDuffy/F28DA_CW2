...
public class PartD {
	...
	public static void main(String[] args) {
		...
		DijkstraShortestPath d = new DijkstraShortestPath(flightGraph,
				startVertex, destinationVertex);
		Iterator path = null;

		List<Flight> foundPath = d.findPathBetween(flightGraph, startVertex,
				destinationVertex);
		if (foundPath != null) {
			...
			System.out.println("Itinerary for: " + startVertex + " to: "
					+ destinationVertex);
			System.out.println();
			System.out.format("%-3s %-5s %-2s %-2s %-6s %-2s", "Leg", "Leave",
					"At", "On", "Arrive", "At");
			System.out.println();
			...
			for (int i = 0; i < flightArray.length; i++) {
				System.out
						.format("%s, %s, %s, %s, %s, %s",
								String.valueOf(i)
										, String.valueOf(flightArray[i]
												.getDepartureTime())
										, flightArray[i].getDeparture()  
										, String.valueOf(flightArray[i]
												.getFlightNumber())
										,flightArray[i].getDestination()
										, String.valueOf(flightArray[i]
												.getArivalTime()));
		...
		if(foundPath != null){
			...
			ListIterator<Flight> i = DijkstraShortestPath.findPathBetween(flightGraph, startVertex,
					destinationVertex).listIterator();
			double sum = 0.00;
			int time = 0;
			while (i.hasNext()) {
	...
	private static SimpleDirectedWeightedGraph<String, Flight> createFlightGraph() {
		...
		String[] airports = new String[] { "auckland", "dubai", "edinburgh",
				"frankfurt", "heathrow", "kuala lumpur", "new york",
				"rio de janeiro", "santiago", "sydney" };

		for (int i = 0; i < airports.length; i++) {
			flightGraph.addVertex(airports[i].toLowerCase());
		}
		
//		String start [] = {"edinburgh", "heathrow", "heathrow", "dubai", "heathrow", "sydney", "dubai", "kuala lumpur", 
//				"dubai", "edinburgh", "kuala lumpur", "sydney", "edinburgh","frankfurt", "sydney", "auckland", "rio de janeiro",
